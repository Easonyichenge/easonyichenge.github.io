...
class TdxService {
    ...
    async getAuthToken() {
        ...
        try {
            ...
            if (e.message.includes('Failed to fetch') || e.name === 'TypeError') {
                console.warn('⚠️ Connection Error: Ensure the local server is running (ruby server.rb).');
                updateSystemStatus('warning', '請啟動 Local Server (ruby server.rb)');
            }
            return null;
        }
    }
    ...
    async fetchData(endpoint) {
        ...
        try {
            ...
            const targetUrl = `${TDX_CONFIG.apiUrl}/${cleanEndpoint}?$format=JSON`;
